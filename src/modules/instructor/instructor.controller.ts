...
// Called right after registration when user selects "Instructor"
// Auth: any logged-in user (role gets promoted to INSTRUCTOR inside the service)
const createInstructorProfile = async (req: Request, res: Response) => {
  try {
    ...
    const result = await instructorService.createInstructorProfile(
      req.body,
      user.id,
    );
    ...
    res.status(201).json({
      success: true,
      message:
        "Instructor profile created. Your application is pending admin approval.",
      data: result,
    });
  } catch (error: any) {
    res.status(400).json({
      success: false,
      message: error.message || "Failed to create instructor profile",
    });
  }
};
...
const getMyProfile = async (req: Request, res: Response) => {
  try {
    ...
    const statusCode =
      error.message === "Instructor profile not found" ? 404 : 500;

    res.status(statusCode).json({
      success: false,
      message: error.message || "Failed to retrieve instructor profile",
    });
  }
};
...
const updateMyProfile = async (req: Request, res: Response) => {
  try {
    ...
    res.status(statusCode).json({
      success: false,
      message: error.message || "Failed to update instructor profile",
    });
  }
};
...
// GET /instructor-profiles/:id  (Public)
...
// GET /instructor-profiles  (Public)
